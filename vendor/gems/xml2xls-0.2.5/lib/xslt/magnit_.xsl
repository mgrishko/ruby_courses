        ...
        <xsl:choose>
          <xsl:when test="count($Item/fnf_{1}:PackagingItem/fnf_{1}:PackagingItemVersion[fnf_{1}:GTINofNextLowerPackagingItem=$parentGTIN])=0">
            <xsl:call-template name="for.item">
              <xsl:with-param name="item" select="exsl:node-set($Item/fnf_{1}:PackagingItem/fnf_{1}:PackagingItemVersion[fnf_{1}:GTIN=$parentGTIN])"/>
              <xsl:with-param name="BI" select="exsl:node-set($BI)"/>
              <xsl:with-param name="block" select="exsl:node-set($block)"/>
            </xsl:call-template>
          </xsl:when>
          <xsl:otherwise>
