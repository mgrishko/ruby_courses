    ...
    <xsl:for-each select="fnf_{1}:Item">
      ...
      <xsl:if test="$biGTIN">
        ...
        <xsl:call-template name="for.item">
          <xsl:with-param name="item" select="fnf_{1}:BaseItem/fnf_{1}:BaseItemVersion"/>
          <xsl:with-param name="biGTIN" select="$biGTIN"/>
          <xsl:with-param name="biPackagingFNF" select="$biPackagingFNF"/>
      ...
      <xsl:if test="$biGTINA">
        ...
        <xsl:call-template name="for.item">
          <xsl:with-param name="item" select="fnf_{1}:Assortment/fnf_{1}:AssortmentVersion"/>
          <xsl:with-param name="biGTIN" select="$biGTINA"/>
          <xsl:with-param name="biPackagingFNF" select="$biPackagingFNFA"/>
