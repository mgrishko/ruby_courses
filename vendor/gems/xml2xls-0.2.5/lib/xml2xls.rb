...
module Xml2xls    
  FILEMANAGER = {
    "xslt" => File.join(File.dirname(__FILE__), 'xslt')
  }
  def self.convert xml, template_name, filename
    ...
    if File.file? File.join(xsl_folder_path, template_name + "_sep.xsl")
      ...
      if File.file? File.join(xsl_folder_path, template_name + "_.xsl") or File.file? File.join(xsl_folder_path, template_name + "_h.xsl") or File.file? File.join(xsl_folder_path, template_name + "_gen.xsl")
        ...
        #out_xml.gsub!(/\s*<\?xml version="1\.0"\?>\s*/, "") TRUE!!!
      else
  ...
  def self.process_xml_node reader, template_name 
    ...
    if File.file? file_path
      ...
      hierarchy_dictionary.to_a.sort{|i,j| i[0] <=> j[0]}.each do |key, value|
  ...
  def self.process_xml_sep_node reader, template_name, xml, fname
    ...
    processed_xmls = []
    leaves.to_a.sort{|i,j| i[0] <=> j[0]}.each do |key, value|
      value.each do |pi|
        ...
        processed_xml = process_text(processed_xml)
        processed_xmls << make_stylesheet(processed_xml, filename + ".xls")
      end
    end
  ...
  def self.process_text text
    temp = text.dup
    count = 0
    if temp.index(/[A-Z]\d{6,6}/)
      temp.gsub!(/<s:Data s:Type="Number">([A-Z]\d{6,6})<\/s:Data>/, "<s:Data s:Type=\"String\">\\1<\/s:Data>")
    end
    @regex_tpls.each do |key, value|
      ...
      temp.scan(regex).each do |match|
        ...
        val = ""
        val = mapping[s] if mapping[s]
        new_val = match[0].gsub(">" + s + "<", ">" + val + "<")
    ...
    temp.scan(@convert_regex).each do |match|
      ...
      next if strs.size < 2
      code = strs[0].strip
      value = BigDecimal.new(strs[1].sub(',', '.'))
      ...
      if code != tocode
        ...
        next if coeff.empty?
        value *= BigDecimal.new(coeff.sub(',', '.'))
      end
      temp.gsub!(/\s*#{@convert_substitution_string.format(s, match[6].strip, tocode, first, second, third)}\s*/, value.to_s('F'))
    end
    return temp
  end
  ...
  def self.get_leaf_pi (reader, suffix)
    ...
    result = {}
    dict.to_a.sort{|i,j| i[0] <=> j[0]}.each do |key, value|
