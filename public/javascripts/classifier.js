...
jQuery.classifier = function(input, options) {
	...
	.keydown(function(e) {
		...
		switch(e.keyCode) {
			...
			case 40: // down
				...
				break;
      case 39: // right
        $j("li",classifier).removeClass("ac_over")
        type = 0;
        break;  
      case 37: // left
        $j("li",classifier).removeClass("ac_over")
  ...
  hideClassifierNow();
	...
	function selectCurrent() {
	  ...
	  switch (type){
	    case 0:
	      li = $j("li.ac_over", groups)[0];
	      if (li){
	        type++;
	        $j(li).removeClass("ac_over").addClass(options.liSelected).addClass(options.loadingClass);
	        subgroups.innerHTML = "";
	        categories.innerHTML = "";
  	      requestClassifierData($j(li).html());
	      }
	      return false;
	    case 1:
  	    li = $j("li.ac_over", subgroups)[0];
  	    if (li){
  	      type++;
  	      $j(li).removeClass("ac_over").addClass(options.liSelected).addClass(options.loadingClass);
  	      categories.innerHTML = "";
	    ...
	    case 2:
  	    type = 0;
	      li = $j("li.ac_over", categories)[0];
	      $j(li).removeClass("ac_over");
        if (!li) {
    			var $li = $j("li", categories);
	...
	function selectCurrentMouse() {
	  var li;
	  if (li = $j("li.ac_over", groups)[0] ){
	    type = 1;
	    $j(li).removeClass("ac_over").addClass(options.liSelected).addClass(options.loadingClass);
	    subgroups.innerHTML = "";
      categories.innerHTML = "";
	    requestClassifierData($j(li).html());
	  }else if (li = $j("li.ac_over", subgroups)[0]) {
	    type = 2;
	    $j(li).removeClass("ac_over").addClass(options.liSelected).addClass(options.loadingClass);
      categories.innerHTML = "";
	    requestClassifierData($j(li).html());
	...
	function dataToDom(data) {
		...
		for (var i=0; i < num; i++) {
			...
			$j(li)
			.hover(function() { 
				  $j("li", ul).removeClass("ac_over"); 
				  $j(this).addClass("ac_over");
  				active = $j("li", ul).indexOf($j(this).get(0));   
			  },
				function() { 
		...
		if (data) {
		  ...
		  switch (type){
		    ...
		    case 1:
		      ...
		      $j('li', groups).removeClass(options.loadingClass);
		      //selectSubgroups(); 
		      break;
		    case 2:
		      $categories.html(dataToDom(data[0]));
		      $j('li', subgroups).removeClass(options.loadingClass);
		      //selectCategories();
		      break;
		  }
        // if ($.browser.msie) {
	...
	// type: on change input -- 0; select groups -- 1; select sub -- 2;
	  ...
	  if( (typeof options.url == "string") && (options.url.length > 0) ){
    	$j.get(makeClassifierUrl(q), function(data) {
    	  data = parseData(data);
